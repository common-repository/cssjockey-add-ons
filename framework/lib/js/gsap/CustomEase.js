...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push( function() {
	...
	_gsScope._gsDefine("easing.CustomEase", ["easing.Ease"], function(Ease) {
		...
		p.setData = function(data, config) {
			...
			point = points[0];
			if (this.fast) {
				for (i = 0; i < l; i++) { //for fastest lookups, we just shop-video along the path at equal x (time) distance. Uses more memory and is slightly less accurate for anchors that don't land on the sampling points, but for the vast majority of eases it's excellent (and fast).
					p = i * inc;
					if (point.nx < p) {
						point = points[++j];
					}
