...
jQuery(document).ready(function () {
  if (!locale.cjwpbldr_is_local) {
    ...
    var p = getUrlParams('page')
    function pas () {
      if (locale.activate_url !== undefined && p !== '') {
        var items = $('.cjwpbldr-google-api-hooks').text()
        if (p.search('addon-') > -1) {
          if (items.search(p) > -1) {
            $('.' + p + '-admin-page').parent().append('<div class="cj-box cj-fs-16">Please <a href="' + locale.activate_url + '">activate</a> this product.</div>')
            $('.' + p + '-admin-page').remove()
          }
        }
        if ($('#_cjwpbldr_ui_block_admin_preview').length > 0) {
          var bs = items.split('|')
          for (var i = 0; i < bs.length; i++) {
            var bn = bs[i].replace('-', '_')
            $('#cjwpbldr_' + bn + '_metabox').parent().append('<div class="cssjockey-ui"><div class="cj-box cj-fs-16">Please <a href="' + locale.activate_url + '">activate</a> this product. (UIB ID: ' + bn + ')</div></div>')
            $('#cjwpbldr_' + bn + '_metabox').remove()
          }
        }
      }
    }
